...
const Product = ({post}) => {
  ...
  return (
    ...
    hover:scale-110 transition duration-300 ease-in gap-3 p-4 mt-10 ml-5 rounded-xl outline outline-slate-200
     hover:shadow-[rgba(0,_0,_0,_0.4)_0px_30px_90px] ">
      <div>
        <p className="text-gray-700 text-lg font-semibold text-left truncate w-40 mt-1 ">{post.title}</p>
      </div>
      <div>
...

      {
        ...
        (<button onClick={removeFromCart} className="text-gray-700 border-2 border-gray-700 rounded-full 
          text-[12px] p-1 px-3 uppercase  font-semibold
